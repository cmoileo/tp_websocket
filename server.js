...
function disconnectClient(targetId, event) {
  const target = clients.get(targetId);
  if (!target) return;
  target.ws.close(1000, event || "admin_disconnect");
}
...
function handleJoin(client, payload) {
  ...
  if (!name || !avatar) {
    sendToClient(client.id, "error", {
      message: "Nom et avatar obligatoires.",
    });
    return;
  }
  client.name = name;
